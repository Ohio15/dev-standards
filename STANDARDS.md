...
**Pre-commit installation:** clone dev-standards (`main`) and run `./install.sh /path/to/repo`. There is no curl one-liner: `install.sh` copies `hooks/`, `workflows/`, `templates/` and `security/` from its own checkout, so a piped script has nothing to copy (the former `.../v1/install.sh` one-liner never worked and was removed 2026-09-11). The `v1` tag is for the reusable release workflows (`release-gates.yml@v1`, moved by `promote-canary.yml`), not for onboarding. Idempotent. Currently installs: `.githooks/commit-msg` (conventional subject + attribution policy, always overwritten) + `.githooks/pre-commit` (dispatcher) + `pre-commit-secret-scan.sh` (gitleaks staged-diff) + `pre-commit-size-guard.sh` (10 MB ceiling) + `pre-commit-healthcheck-lint.sh` + `pre-commit-tests.sh`. Sets `core.hooksPath=.githooks`. Version-bump-reminder hook is a TO-BE addition (IMPL-2).

**Conventional commits:** `feat:`, `fix:`, `chore:`, `docs:`, `refactor:`, `test:`, `perf:`, `security:`, `build:`, `ci:`, with optional `(scope)` and breaking `!` per the Conventional Commits spec (`chore(deps):` from Dependabot is valid). Enforced locally by `hooks/commit-msg` (block). Enforcement plan for the server side: PR titles via GitHub branch ruleset; CI gate on protected branches (block).

**Commit attribution (decided 2026-09-11, dev-standards #5):** agent-authored commits carry the `Co-Authored-By:` and `Claude-Session:` trailers the Claude Code harness emits; the hook allows them, so `--no-verify` is never needed for attribution. `Co-Authored-By:` is a standard git trailer and is equally valid for human co-authors. PR-description boilerplate (`Generated with …`, `Auditor:`) stays out of commit messages. Between 2026-04-27 and 2026-09-11 the hook had no versioned source and the per-repo copies diverged (some rejecting the trailers, some allowing them); `hooks/commit-msg` is now the only source and `install.sh` overwrites the copy on every run.
...
- **Worktrees live in `D:/Worktrees/<repo>/<branch-with-slashes-as-dashes>`**, never under `D:/Projects` at any depth (the 2026-08-04 `truck-mcp/worktrees/` convention is retired). Create with `git worktree add D:/Worktrees/<repo>/<branch> <branch>`; a finished worktree (clean, branch merged) is removed the same session with `git worktree remove` then `git worktree prune`. Worktrees a Claude session creates in its scratchpad are fine (they die with the session) but must be pruned from the repo afterwards. A repo whose `package.json` links a sibling by relative path (`"file:../cortex-core"`) gets ONE symlink beside its worktrees (`D:/Worktrees/cortex-hooks/cortex-core -> D:/Projects/cortex/cortex-core`) so every worktree resolves it; the symlink is the only non-worktree entry allowed there.
